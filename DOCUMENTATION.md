...
### 3.2 `project_to_sieve()` 与 `add_sieve_projection()`
...
| 参数 | 默认值 | 物理含义 |
|------|--------|---------|
| `x_z_coefficient` | 253.0 | 筛板到靶面距离（cm）。改变此值等效于修改筛板的 Z 位置假设，直接影响 `sieve_x` 的计算精度 |
| `y_dp_linear` | −0.019 | $\delta$（动量偏差）对 sieve_y 的一阶贡献系数（色散修正） |
...
> **参数影响分析**：
> - `x_z_coefficient` 若偏差 1 cm，则 $x'_{tar} \sim 10^{-2}$ rad 量级的角度会产生约 2.5 mm 的 sieve_x 偏移——这对筛孔识别有显著影响。
> - `y_ph_coefficient` 的值 213.0 = 138.0 + 75.0，分别来自主弯转磁铁后的路径长度和修正量，此系数偏差会使整个 sieve_y 图案在水平方向整体平移。
...
| `sieve_x_range` | (−20.0, 20.0) | sieve_x 的有效范围（cm），超出范围的事例被过滤 |
| `sieve_y_range` | (−20.0, 20.0) | sieve_y 的有效范围（cm） |
...
| 参数 | 默认值 | 含义与影响 |
|------|--------|-----------|
| `x_col` | `'sieve_x'` | X 坐标列名 |
| `y_col` | `'sieve_y'` | Y 坐标列名 |
...
| `xlim` | (−20.0, 20.0) | X 轴范围（cm）。应与 `sieve_x_range` 一致 |
...
| `visualize_sieve_plane()` | 筛板平面坐标的 2D 热图 |
...
| `sieve_x_range` | (−20.0, 20.0) | tuple | sieve_x 过滤范围（cm） |
| `sieve_y_range` | (−20.0, 20.0) | tuple | sieve_y 过滤范围（cm） |
...
| 参数 | 默认值 | 类型 | 说明 |
|------|--------|------|------|
| `x_col` | `'sieve_x'` | str | X 坐标列名 |
| `y_col` | `'sieve_y'` | str | Y 坐标列名 |
